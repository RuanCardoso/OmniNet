...
namespace Omni.Core
{
    internal sealed class UdpClient : UdpSocket
    {
        ...
        /// </summary>
        internal byte[] AesKeyToEncrypt { get; set; }
        /// <summary>
        /// Gets or sets the AES key used for decryption.
        /// </summary>
        internal byte[] AesKeyToDecrypt{ get; set; }
        ...
        private IEnumerator Connect()
        {
            ...
            while (!IsConnected)
            {
                if (RSAKeysExchange)
                {
                    ...
                    IOHandler.Write128Bits(AesKeyToEncrypt);
                    OmniLogger.Print($"AES KEY SEND: {BitConverter.ToString(AesKeyToEncrypt)}");
                ...
                case MessageType.Connect:
                    {
                        if (!IsConnected)
                        {
                            ...
                            AesKeyToDecrypt = IOHandler.Read128Bits();
                            OmniLogger.Print($"AES KEY REC FROM SERVER: {System.BitConverter.ToString(AesKeyToDecrypt)}");
                            IsConnected = true;
