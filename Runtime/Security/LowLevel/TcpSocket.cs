...
{
	/// <summary>
	/// Represents a TCP socket used for asynchronous network communication. LOW LEVEL!
	/// </summary>
	internal class TcpSocket
	{
		private bool isServer;
		private Socket tcpSocket;
		private SocketClient clientStream;
				...
				if (isServer)
				{
					...
					while (!cts.IsCancellationRequested)
					{
						try
						{
							...
							acceptedSocket.ExclusiveAddressUse = true;
		...
		internal async void Connect(UdpEndPoint remoteEndPoint)
		{
			try
			{
				...
				InternalEventHandler.FireTcpConnected();
		...
		{
			DataIOHandler data = new(RSACryptography.IOHandlerSize);
			try
			{
		...
		internal void FriendlyDisconnection(UdpEndPoint remoteEndPoint)
		{
			if (isServer)
			{
				if (tcpSockets.Remove(remoteEndPoint, out SocketClient socketClient))
				{
					OmniLogger.Print($"TCP: The connection has been gracefully released: {remoteEndPoint}");
					socketClient.Free();
				}
				else
				{
					OmniLogger.PrintError("Attempted to release a connection that is not currently established.");
				}
			}
		}
