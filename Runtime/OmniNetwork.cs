...
namespace Omni.Core
{
	...
	public class OmniNetwork : MonoBehaviour
	{
		...
		{
#if UNITY_SERVER && !UNITY_EDITOR
			// Skip 100 lines in console
			// This is necessary to avoid the user from seeing the initialization messages
			// Console.Clear(); not working ):
			for (int i = 0; i < 100; i++)
			{
				OmniLogger.Print("\r\n");
			}
		...
		private void Update()
		{
			...
			{
				DataIOHandler dataIOHandler = DataIOHandler.Get(MessageType.StressTest);
				dataIOHandler.Write("Hello World!");
				tcpClient.SendToServer(dataIOHandler);
			}
