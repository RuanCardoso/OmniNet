...
namespace MessagePack.Unity.Editor
{
	internal class MessagePackWindow : EditorWindow
	{
		...
		private async Task<string> InitCodeGen()
		{
			var commnadLineArguments = mpcArgument.ToString();
		...
		public static void GenerateAddResolverInstance(string path, string className, string log)
		{
			try
			{
				...
				using (StreamWriter writer = new StreamWriter(Path.Combine(path, fileName), false))
				{
					...
					writer.WriteLine("using Omni.Internal;");
					writer.WriteLine("using Omni.Resolvers;");
					writer.WriteLine();
					writer.WriteLine($"public class {fileName.Replace(".cs", "")}");
					writer.WriteLine("{");
					writer.WriteLine("    [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]");
					writer.WriteLine("    static void Load()");
					writer.WriteLine("    {");
					writer.WriteLine($"        NetworkHelper.AddResolver({className});");
					writer.WriteLine("    }");
					writer.WriteLine("}");
				}
				OmniLogger.Print("Success: Click to see details: " + log);
			}
			catch
			{
				OmniLogger.PrintError("Error: Click to see details: " + log);
			}
		}
					...
					if (selObj != null)
					{
						...
						if (Directory.Exists(outputPath))
						{
							...
							if (asmDefs.Length > 0)
							{
								...
								string log = await window.InitCodeGen();
								GenerateAddResolverInstance(path, $"{argument.ResolverName}.Instance", log);
								AssetDatabase.Refresh();
								window = null;
							}
